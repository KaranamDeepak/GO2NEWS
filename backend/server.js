...
function extractImage(item) {
  try {
    ...
    /* media content */
    if (item.media?.content?.url && item.media.content.url.startsWith("http")) {
...
async function classifyAI(article) {
  try {
    ...
    return {
      category: "general",
      state: article.state || "world",
      summary: (article.description || article.title || "").slice(0, 500),
    };
  }
}
...
async function fetchAllNews() {
  ...
  return removeDuplicates([...apiNews, ...rssNews]);
}
...
/* =====================================
   START SERVER
===================================== */
// connectDB(); // ✅ moved before listen (optional, but fine)
app.listen(5000, () => {
